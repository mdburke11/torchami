...
struct complete_graph {
  // struct to hold the graph and associated graph prefactor together in this
  // example
...
complete_graph read_diagram(TamiGraph tamig, std::string folder, int ord,
                            int group, int n);
std::vector<complete_R0_graph> read_ct_diagrams(TamiGraph tamig,
                                                TamiGraph::graph_t diagram,
                                                int order, int max_ct);
...
public:
  ...
  flat_mc_integrator(at::TensorOptions options_, int N_frequencies,
                     int max_bs) {
    options = options_;
...
public:
  ...
  void update_ext_vars(ext_vars new_evars) {
    ...
    evars = new_evars;
    // avars.frequency_[0][-1] = TamiBase::complex_double(new_evars.reW,
    // new_evars.imW);
  ...
  void update_integrand(at::Tensor& k) {
    ...
    for (double q : evars.k) {
      cols.push_back(at::full({at::size(k, /*dim*/ 0), 1}, q, options));
    }
    ...
    for (at::Tensor x :
         at::split(combined, /*size of split*/ dim, /*dim to split on*/ 1)) {
