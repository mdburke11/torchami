...
// right now this only does real valued functions, so all things are evaluated twice
// but it should be an easy fix to make it for complex.
...
at::Tensor flat_mc_integrator::prepInput(int length, integ_domain& domain){
    ...
    at::TensorOptions opts = at::TensorOptions().dtype(at::kDouble).device(options.device());
...
integration_result flat_mc_integrator::integrate(std::function<at::Tensor(at::Tensor)> fn, int dim, int N, integ_domain domain){
    ...
    try{
        if (domain.size() != dim){
            throw std::invalid_argument("Dimension and length of integration domain do not match!");
        }
    }
    ...
    // then perform one last evaluation with whats left. Note we dont re evaluate for nans here, but 
