...
TamiBase::ami_vars prep_ext(int ord, ext_vars evars, at::Device dev) {
  ...
  at::TensorOptions options =
      at::TensorOptions().dtype(at::kDouble).device(dev);
  at::TensorOptions complexoptions =
      at::TensorOptions().dtype(at::kComplexDouble).device(dev);

  TamiBase::energy_t energy = at::zeros({default_batchsize, ord + 1}, options);
  TamiBase::frequency_t frequency =
      at::zeros({default_fbatchsize, ord + 1}, options);
