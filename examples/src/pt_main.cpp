...
void default_example() {
  ...
  TamiBase::g_prod_t R02 =
      construct_multipole_example(); // construct_example_J();//construct_multipole_example();//construct_example1_bose();
  TamiBase::ami_vars avars2 = construct_4ord_ext_multipole_example(
      PT, energy_batchsize,
      freq_batchsize); // construct_ext_example_J();//construct_4ord_ext_multipole_example();//construct_ext_example1_bose();
  ...
  std::chrono::nanoseconds d2 =
      std::chrono::duration_cast<std::chrono::nanoseconds>(diff2);
...
void graph_library_example() {
  ...
  // this is an example of using the libtami_graph library to evaluate the
  // graphs in the ggm_examples folder.

  std::string foldername =
      "../ggm_examples"; // folder that holds all the graph files

  TamiBase::graph_type g_type =
      TamiBase::Sigma; // Specify graph type: self energy diagram
  ...
  int max_ord =
      6; // max order diagram in the graph folder if order 6 by default
...
void example2() {
  ...
  // Number of parameters to evaluate - number of frequencies and num of
  // energies to eval at a time
  int fbatch_size = 1;
  int ebatch_size = 10;

  std::cout << std::endl
            << "-_-_-_ Example - Second Order _-_-_-" << std::endl
            << std::endl;
  ...
  TamiBase::ami_vars avars = construct_ext_example2(
      ami, ebatch_size, fbatch_size); // Sets 'external' parameter values
  ...
  std::chrono::nanoseconds d1 =
      std::chrono::duration_cast<std::chrono::nanoseconds>(diff1);

  std::cout << "Construction took " << d1.count() << " nanoseconds"
            << std::endl;
  ...
  at::Tensor term_val =
      ami.evaluate(test_amiparms, amiterms,
                   avars); // Evaluate the term-by-term result for external
                           // values in 'avars'. Note that the test_amiparms is
  ...
  std::cout << "Term result was " << format_r2_tensor(term_val)
            << "Shape: " << term_val.sizes() << std::endl;
  std::cout << "Evaluation took " << d2.count() << " nanoseconds" << std::endl;
...
void example1_bose() {
  ...
  std::cout << std::endl
            << "-_-_-_ Example - First Order For Bosonic _-_-_-" << std::endl
            << std::endl;
  ...
  TamiBase::ami_vars avars = construct_ext_example1_bose(
      ami, ebatch_size, fbatch_size); // Sets 'external' parameter values
...
void example4() {
  ...
  std::cout << std::endl
            << "-_-_-_ Example - Fourth Order _-_-_-" << std::endl
            << std::endl;
  ...
  // Problem setup (see ami_example.cpp)
  TamiBase::g_prod_t R0 =
      construct_multipole_example(); // Sets initial integrand
  TamiBase::ami_vars avars = construct_4ord_ext_multipole_example(
      ami, ebatch_size, fbatch_size); // Sets 'external' parameter values
  ...
  at::Tensor term_val = ami.evaluate(test_amiparms, amiterms,
                                     avars); // Evaluate the term-by-term result
                                             // for external values in 'avars'.
...
void example6() {
  ...
  std::cout << std::endl
            << "-_-_-_ Example - Sixth Order _-_-_-" << std::endl
            << std::endl;
  ...
  TamiBase::ami_vars avars = construct_ext_example6(
      ami, ebatch_size, fbatch_size); // Sets 'external' parameter values
...
void example9() {
  ...
  std::cout << std::endl
            << "-_-_-_ Example - Ninth Order _-_-_-" << std::endl
            << std::endl;
  ...
  TamiBase::ami_vars avars = construct_ext_example_J(
      ami, ebatch_size, fbatch_size); // Sets 'external' parameter values
