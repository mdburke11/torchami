...
class integration_result:
    ...
    def __init__(self, sum: torch.tensor, sum2: torch.tensor, N: torch.tensor):
        ...
        self.error: torch.tensor = torch.sqrt((self.avg_ans2 - self.ans**2) / self.N)
...
class flat_mc_integrator:
    ...
    def integrate(self, fn: Callable[[torch.tensor], torch.tensor], dim: int, N: int, 
        ...
        if self.N_freq > 1:
            ...
            remaining : torch.tensor = N * torch.ones(self.N_freq, device=self.device)
            ...
            while remaining.min() > self.Max_batch: # take full batchs
                ...
                eval: torch.tensor = fn(x) # tensor of dim N_freq by batch_size then sum along dim=1 (rows)
