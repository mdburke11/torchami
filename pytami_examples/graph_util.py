...
def main() -> None:
    ...
    second_ord_sigma = read_diagram(g, folder, order, group, n)
    ...
    max_ct = 2
    ct_R0 = read_ct_diagrams(g, second_ord_sigma, max_ct)
...
def read_ct_diagrams(ptg : pytami.TamiGraph, diagram: pytami.TamiGraph.trojan_graph,
                     max_ct: int) -> list[pytami.TamiBase.g_prod_t]:
...
def read_diagram(ptg : pytami.TamiGraph, folder : str, ord : int,
                  group : int, n : int) -> pytami.TamiGraph.trojan_graph:
    ...
    diagram: pytami.TamiGraph.trojan_graph = pytami.TamiGraph.trojan_graph(gg, n)
