...
def second_ord_c1_a() -> None:
    ...
    for _ in range(10):
        ...
        E_REG: float = 0
        N_INT: int = order
        parms: pytami.TamiBase.ami_parms = pytami.TamiBase.ami_parms(
            N_INT, E_REG)
        ami.construct(N_INT, R0, ftout)

        integrand: rpt.RPT_integrand = rpt.RPT_integrand(
            ami, R0, prefactor, avars, ftout, parms, ext.epsilon_2D,
            rpt.dampt_eps, True, evars)
        ...
        value = flat_mc.integrate(integrand,
                                  dim=4,
                                  N=6_999_999,
