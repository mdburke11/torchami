...
def mat_axis() -> None:
    ...
    max_ct: int = 1
    ct_R0: list[pytami.TamiBase.g_prod_t]
    prefactors: list[float]
    ct_R0, prefactors = gru.read_ct_diagrams(g, second_ord_sigma, order,
                                             max_ct)
    ...
    for i in range(1, len(All_R0)):
        ...
        re_integrand = rpt.RPT_integrand(ami, R0, prefactor, avars, ftout,
                                         parms, ext.epsilon_2D, constant_z,
                                         True, evars)
        im_integrand = rpt.RPT_integrand(ami, R0, prefactor, avars, ftout,
                                         parms, ext.epsilon_2D, constant_z,
                                         False, evars)
        ...
        with open(fname, "a") as f:
            ...
            for n in range(MAX_MAT_FREQ):
                ...
                re_integral_value = flat_mc.integrate(
                    re_integrand,
                    dim=4,
                    N=N,
                    integration_domain=[[0, 2 * torch.pi]] * 4)
                im_integral_value = flat_mc.integrate(
                    im_integrand,
                    dim=4,
                    N=N,
                    integration_domain=[[0, 2 * torch.pi]] * 4)
                ...
                print(
                    f"{n} {evars.imW} {re_integral_value.ans} {re_integral_value.error} {im_integral_value.ans} {im_integral_value.error}"
                )
                f.write(
                    f"{n} {evars.imW} {re_integral_value.ans} {re_integral_value.error} {im_integral_value.ans} {im_integral_value.error}\n"
                )
