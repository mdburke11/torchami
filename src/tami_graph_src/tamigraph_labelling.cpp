...
// if(ami_parameters.TYPE_!=TamiBase::density && ami_parameters.TYPE_!= TamiBase::doubleocc){
...
void TamiGraph::find_unlabelled_fermionic_edges(graph_t &g, edge_vector_t &vector){
...
if( g[*ei].g_struct_.stat_==TamiBase::Fermi && g[*ei].label==unlabelled){
vector.push_back(*ei);
}
////std::cout << "Edge " << *ei << " of stat_type "<< g[*ei].g_struct_.stat_ << std::endl;
...
//if( g[*ei].g_struct_.stat_==TamiBase::Fermi){  /// DONT need THIS LOGIC? Bosonic lines get same reset of labels.
...
if(graph_type == TamiBase::Pi_phuu || graph_type == TamiBase::Pi_phud || (graph_type== TamiBase::doubleocc) || ami_parameters.TYPE_==TamiBase::Pi_ppud || ami_parameters.TYPE_==TamiBase::Pi_ppuu){
...
}else if(graph_type==TamiBase::density || graph_type==TamiBase::Greens || graph_type==TamiBase::DOS || graph_type==TamiBase::ENERGY){
...
void TamiGraph::label_extern_legs(edge_vector_t &extern_vect_list,graph_t &g){
	...
	for (int i=0; i< extern_vect_list.size(); i++){
		...
		if(graph_type==TamiBase::density || graph_type==TamiBase::DOS || graph_type==TamiBase::Greens|| graph_type==TamiBase::ENERGY){
...
void TamiGraph::label_and_find_tadpoles_ami(graph_t &g, vertex_vector_t &tp_vec, vertex_vector_t &tp_conn_vec, edge_vector_t &tp_bose_edges){
...
void TamiGraph::assign_cons_label(graph_t &g, vertex_t &vin, edge_t &fermi_one, edge_t &fermi_two, edge_t &bose){
...
if( (graph_type == TamiBase::Pi_phuu) || (graph_type == TamiBase::Pi_phud) || (graph_type==TamiBase::doubleocc)|| graph_type==TamiBase::Pi_ppud || graph_type==TamiBase::Pi_ppuu || graph_type==TamiBase::FORCE){
...
void TamiGraph::find_external_vertices(graph_t &g, vertex_vector_t &v, edge_vector_t &edges){
...
void TamiGraph::find_tadpoles(graph_t &g, vertex_vector_t &tp_vec, vertex_vector_t &tp_conn_vec, edge_vector_t &tp_bose_edges, edge_vector_t &edge_a, edge_vector_t &edge_b ){
...
void TamiGraph::delete_legs(graph_t &g, vertex_vector_t &v, edge_vector_t &edges){
...
bool TamiGraph::label_consv_momentum(graph_t &g, vertex_t &vin, vertex_vector_t &labelled_adj_vertices, vertex_vector_t &unlabelled_adj_vertices, edge_vector_t &labelled_edges, edge_vector_t &unlabelled_edges){
...
// both fermionic lines
	if (g[unlabelled_edges[0]].g_struct_.stat_==TamiBase::Fermi && g[unlabelled_edges[1]].g_struct_.stat_==TamiBase::Fermi){ 	
...
// first fermi second bose
	if (g[unlabelled_edges[0]].g_struct_.stat_==TamiBase::Fermi && g[unlabelled_edges[1]].g_struct_.stat_==TamiBase::Bose){
...
// first bose second fermi 
	if (g[unlabelled_edges[0]].g_struct_.stat_==TamiBase::Bose && g[unlabelled_edges[1]].g_struct_.stat_==TamiBase::Fermi){
...
// the order matters
void TamiGraph::get_adjacent_vertex_stat(graph_t &g, vertex_t &vin, vertex_t &next, edge_t &eout,  TamiBase::stat_type stat){
...
void TamiGraph::sort_labelled_unlabelled_adjacent(graph_t &g, vertex_t &vin,  vertex_vector_t &labelled_adj_vertices,vertex_vector_t &unlabelled_adj_vertices, edge_vector_t &labelled_edges, edge_vector_t &unlabelled_edges){
...
void TamiGraph::find_internal_fermionic_edges(graph_t &g, edge_vector_t &vector){
