...
void TamiGraph::combinations_repetition(int n, int r,
...
void TamiGraph::combinations_repetition_util(
...
double TamiGraph::get_prefactor(graph_t &g, int order) {
  ...
  if (ami_parameters.TYPE_ == TamiBase::Pi_ppud ||
      ami_parameters.TYPE_ == TamiBase::Pi_ppuu) {
    fermi_loops--;
  }
  if (ami_parameters.TYPE_ == TamiBase::Sigma ||
      ami_parameters.TYPE_ == TamiBase::Greens ||
      ami_parameters.TYPE_ == TamiBase::density ||
      ami_parameters.TYPE_ == TamiBase::DOS ||
      ami_parameters.TYPE_ == TamiBase::Bare) {
    if (order != 0) {
      fermi_loops--;
    }
  }
  ...
  for (int ord = 0; ord < ggm.size(); ord++) {
    ...
    for (int m = 0; m < ggm[ord].size(); m++) {
      for (int i = 0; i < ggm[ord][m].graph_vec.size(); i++) {
        ...
        check++;
        if (ord > 2) {
          // std::cout<<"Graph ord "<<ord<<" group "<< m <<" num "<<i<<" has
          // n_labels="<<ggm[ord][m].labels[i].size()<<std::endl;
        }
      }
    ...
    std::cout << "There were " << check << " graphs and " << gpcheck
              << " pairs at order " << ord << " in " << ggm[ord].size()
              << " groups." << std::endl;
    // std::cout<<"There were "<<check<<" graphs of order "<< ord<< " in "<<
    // ggm[ord].size()<<" groups."<<std::endl;
  }
}

void TamiGraph::find_bose_fermi_edges(graph_t &g, edge_vector_t &bose,
                                      edge_vector_t &fermi) {
...
int TamiGraph::graph_order(graph_t &g) {
  ...
  // }
  // std::cout<<"In graph order function we have "<<edges.size()<<"
  // "<<eff_bose<<std::endl;

  return edges.size(); //+eff_bose;
}

void TamiGraph::find_force_LR_vertices(graph_t &g, vertex_vector_t &in_vv,
...
void TamiGraph::fix_force_labels(graph_t &g, vertex_vector_t &in_vv,
                                 vertex_vector_t &out_vv) {
  ...
  if (success) {
    ...
    throw std::runtime_error("Failed to find force line");
  }
}
...
vertex_vector_t &out_vv){
  ...
  std::cout<<"In vertices are "<< g[in_vv[0]].index_<<"
"<<g[in_vv[1]].index_<<std::endl; std::cout<<"out vertices are "<<
g[out_vv[0]].index_<<" "<<g[out_vv[1]].index_<<std::endl;
...
// At this point it was succesful so we found a path and can set the external
lines=1
...
void TamiGraph::find_path_between_vertices(graph_t &g, vertex_t &v1,
                                           vertex_t &v2, bool &success,
                                           edge_vector_t &final_ev,
  ...
  // std::cout<<"Keep going is "<< keep_going<<" success is "<<
  // success<<std::endl;
  ...
  // std::cout<<g[source(final_ev[i],g)].index_
  // <<"-"<<g[target(final_ev[i],g)].index_ <<std::endl;
  // }
}

void TamiGraph::put_back_legs(graph_t &g, vertex_vector_t &in_vv,
                              vertex_vector_t &out_vv) {
  int alpha_size = 0;
  int eps_size = 0;
  ...
  for (int i = 0; i < in_vv.size(); i++) {
    ...
    edge_t this_edge =
        add_edge(new_vert, in_vv[i], edge_info(TamiBase::Bose), g).first;
  ...
  for (int i = 0; i < out_vv.size(); i++) {
    ...
    vertex_t new_vert = add_vertex(g);
    edge_t this_edge =
        add_edge(out_vv[i], new_vert, edge_info(TamiBase::Bose), g).first;
...
int TamiGraph::fermi_connected_components(graph_t &g) {
  ...
  std::vector<boost::graph_traits<cc_graph_t>::vertex_descriptor> v1(
      num_vertices(gc));
...
void TamiGraph::ggm_label(gg_matrix_t &ggm, int min) {
  ...
  for (int ord = min; ord < ggm.size(); ord++) {
    std::cout << "Producing ggm labels at order " << ord << " with size "
              << ggm.size() << std::endl;
    ...
    for (int group = 0; group < ggm[ord].size(); group++) {
      ...
      for (int graph = 0; graph < ggm[ord][group].graph_vec.size(); graph++) {
        ...
        std::cout << "Labelling graph ord" << ord << " group " << group
                  << " num " << graph << std::endl;
        bool success = true;
...
void TamiGraph::find_internal_edges_stat(graph_t &g, edge_vector_t &vector,
...
// NOTE: Due to force diagrams, need to not associate an external bosonic line
// with the actual tadpole
void TamiGraph::find_bosonic_edge_on_three_pointvert(graph_t &g, vertex_t v,
                                                     edge_t &bose_edge) {
