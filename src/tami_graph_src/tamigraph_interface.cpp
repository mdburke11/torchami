...
if( graph_type== TamiBase::density || graph_type== TamiBase::Greens || graph_type== TamiBase::DOS  ){
...
void TamiGraph::extract_bose_alphas(graph_t g, std::vector<TamiBase::alpha_t> &bose){
...
boost::graph_traits<graph_t>::vertex_iterator vi, vi_end;
  ...
  add_edge(source,target, edge_info(TamiBase::stat_type(statvec[edge]), -1, spin_type(spinvec[edge])), loaded);
...
void TamiGraph::pair_read(std::string filename, git_pair &p){
	...
	boost::graph_traits<graph_t>::vertex_iterator vi, vi_end;
	  ...
	  add_edge(source,target, edge_info(epsvec[edge], alphavec[edge], TamiBase::stat_type(statvec[edge]), -1, spin_type(spinvec[edge])), loaded);
	}
	if(pairID[edge]==1){
	boost::graph_traits<graph_t>::vertex_descriptor source,target;
	...
	boost::graph_traits<graph_t>::vertex_iterator vi, vi_end;
	  ...
	  add_edge(source,target, edge_info(epsvec[edge], alphavec[edge], TamiBase::stat_type(statvec[edge]), -1, spin_type(spinvec[edge])), g2);
	}
...
void TamiGraph::generate_sigma_ct( graph_t &g_in, std::vector< graph_t> &ct_vec, int maxdots){
...
if(graph_type==TamiBase::density|| graph_type==TamiBase::DOS || graph_type==TamiBase::Greens){
  find_fermionic_edges(g_in, fermi_edges);
}else{
...
void TamiGraph::insert_chain(graph_t &g_in, graph_t &ctg,  edge_t &e, int length){
...
if(	(ctg[source(*ei,ctg)].index_== g_in[source(e,g_in)].index_) && (ctg[target(*ei,ctg)].index_== g_in[target(e,g_in)].index_) && g_in[*ei].g_struct_.stat_==TamiBase::Fermi){
vA=	source(*ei,ctg);
vB= target(*ei,ctg);
	cedge=*ei;
}
