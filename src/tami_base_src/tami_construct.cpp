...
void TamiBase::construct(int N_INT, TamiBase::g_prod_t &R0,
                         ft_terms &terms_out) {
  terms_out.clear();
...
void TamiBase::integrate_step(int index, ft_terms &in_terms,
                              ft_terms &out_terms) {
...
void TamiBase::term_integrate_step(int index, ft_term &in_term,
                                   ft_terms &out_terms) {
  ...
  // if(FT.is_empty_ft(in_term.ft_)){std::cout<<"Input term is
  // empty"<<std::endl;
  ...
  // std::cout<<"Factorized Terms are "<<
  // factorized_terms.size()<<"------------------"<<std::endl;
  // std::cout<<pretty_print_ft_terms(out_terms)<<std::endl;
  ...
  for (int i = 0; i < in_terms.size(); i++) {
    ...
    for (int j = i + 1; j < in_terms.size(); j++) {
      ...
      if (g_prod_equiv(in_terms[i].g_prod_, in_terms[j].g_prod_, junk)) {
        // std::cout<<"Terms equiv "<< i<<" and "<<j<<" with
        // sign="<<junk<<std::endl;
...
bool TamiBase::g_prod_equiv(TamiBase::g_prod_t &gp1, TamiBase::g_prod_t &gp2,
                            int &sign) {
  ...
  // if it clears all the terms then it is true these are equivalent within an
  // overall prefactor of sign
...
std::string TamiBase::pretty_print_ft_term(ft_term &ft) {
  ...
  ss << "[" << ft.sign_ << "]" << FT.pretty_print(ft.ft_)
     << pretty_print_gprod(ft.g_prod_);
...
void TamiBase::terms_to_ftterms(TamiBase::terms &in_terms,
                                ft_terms &out_terms) {

  // look at each term and find unique G's and references and their prefactors.
  // This is already done by the terms factorization functions. Going to do it
  // again to get my head around what is happening.
  out_terms.clear();
  ...
  // if size is more than one then we need to identify common factors - this was
  // already done with the factorize_Rn function

  TamiBase::Ri_t Ri;

  convert_terms_to_ri(
      in_terms,
      Ri); // this should take the terms and pull out just the producs of G's.
  ...
  for (int i = 0; i < Eval_list.size(); i++) {
    ...
    for (int j = 0; j < Eval_list[i].size(); j++) {
      ...
      // std::cout<<"Prefactor mult is
      // "<<in_terms[Eval_list[i][j].first].sign*double(Eval_list[i][j].second)<<std::endl;

      TamiBase::FermiTree::fermi_tree_t ft;

      FT.plist_to_ft(in_terms[Eval_list[i][j].first].p_list,
                     in_terms[Eval_list[i][j].first].sign *
  ...
  for (int i = 0; i < R.size(); i++) {
    if (R[i].alpha_[index] != 0) {
      ...
      if (drop_matsubara_poles) {
        ...
        // At this point if the energy is all zeros then if the nucount is even,
        ...
        if (!non_zero) {
          ...
          // int nucount = std::count(pole.alpha_.begin(), pole.alpha_.end(),
          // 0);
          if (nucount % 2 == 0) {
            non_zero = true;
          }
