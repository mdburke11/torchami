...
void TamiBase::FermiTree::initialize_ft(fermi_tree_t &ft, operation op) {
  ...
  if (num_vertices(ft) > 0) {
    throw std::runtime_error("Attempted to initialize a non-empty fermi tree");
  }
...
void TamiBase::FermiTree::get_roots(fermi_tree_t &ft,
...
void TamiBase::FermiTree::plist_to_ft(TamiBase::pole_array_t &plist,
...
void TamiBase::FermiTree::get_next_level(fermi_tree_t &ft1, vertex_t &root1,
...
void TamiBase::FermiTree::print_graph(fermi_tree_t &ft) {
  ...
  if (num_edges(ft) == 0) {
    if (num_vertices(ft) != 1) {
      throw std::runtime_error("multiple disconnected vertices");
    }
    boost::graph_traits<fermi_tree_t>::vertex_iterator vi, vie;
    for (boost::tie(vi, vie) = vertices(ft); vi != vie; ++vi) {

      std::cout << "|" << ft[*vi].index_ << " " << ft[*vi].operation_
                << " pre:" << ft[*vi].prefactor_ << "| " << std::endl;
    }
  }
  ...
  for (boost::tie(ei, eie) = edges(ft); ei != eie; ++ei) {
    ...
    if (ft[target(*ei, ft)].operation_ != 2) {
      std::cout << "|" << ft[source(*ei, ft)].index_ << " "
                << ft[source(*ei, ft)].operation_ << "| "
                << " pre:" << ft[source(*ei, ft)].prefactor_ << " ---> |"
                << ft[target(*ei, ft)].index_ << " "
                << ft[target(*ei, ft)].operation_ << std::endl;
    } else {
      std::cout << "|" << ft[source(*ei, ft)].index_ << " "
                << ft[source(*ei, ft)].operation_ << "| "
                << " pre:" << ft[source(*ei, ft)].prefactor_ << " ---> |"
                << ft[target(*ei, ft)].index_ << " "
                << ft[target(*ei, ft)].operation_ << " pole "
                << pretty_print_pole(ft[target(*ei, ft)].pole_)
                << " m=" << ft[target(*ei, ft)].pole_.der_
                << " pre:" << ft[target(*ei, ft)].prefactor_ << std::endl;
    }
  }
}

void TamiBase::FermiTree::copy_vertex(vertex_t &root1, vertex_t &root2,
                                      fermi_tree_t &ft1, fermi_tree_t &ft2,
                                      std::pair<vertex_t, vertex_t> &map) {

  vertex_t newv = add_vertex(
      vertex_info(ft1[root1].pole_, ft1[root1].value_, ft1[root1].operation_,
                  ft1[root1].depth_, ft1[root1].prefactor_),
      ft2);
...
void TamiBase::FermiTree::copy_level(
    vertex_t &root1, vertex_t &root2, fermi_tree_t &ft1, fermi_tree_t &ft2,
...
void TamiBase::FermiTree::copy_tree(fermi_tree_t &ft1, fermi_tree_t &ft2,
                                    vertex_t &root1, vertex_t &root2) {
  ...
  do {
    ...
    for (int i = 0; i < map_vec.size(); i++) {
      ...
      for (int m = 0; m < this_map_vec.size(); m++) {
        next_map_vec.push_back(this_map_vec[m]);
      } // for some reason can't use std::copy with std::pair
    }

    if (next_map_vec.size() == 0) {
      cont = false;
    }
...
TamiBase::FermiTree::fermi_tree_t
TamiBase::FermiTree::add_ft(fermi_tree_t ft1, fermi_tree_t ft2) {
  ...
  // check if root of ft1 or ft2 has operation==0 if so put copy the other tree
  // there
  ...
  // std::cout<<"Prefactors are "<<ft1[root1].prefactor_<<" and "
  // <<ft2[root2].prefactor_<<std::endl;
...
TamiBase::FermiTree::fermi_tree_t
TamiBase::FermiTree::mult_ft(fermi_tree_t ft1, fermi_tree_t ft2) {
...
void TamiBase::FermiTree::mult_prefactor(fermi_tree_t &ft, double sign) {
  ...
  // std::cout<<"In mult we have root with prefactor
  // "<<ft[root].prefactor_<<std::endl;
  ...
  if (ft[root].operation_ == 0) {
    ...
    for (int i = 0; i < level.size(); i++) {
      // std::cout<<"Setting prefactor to
      // "<<ft[level[i]].prefactor_*sign<<std::endl;
...
std::string TamiBase::FermiTree::pretty_print(fermi_tree_t &ft, vertex_t &v) {
  ...
  if (ft[v].operation_ == add) {
    ...
    if (ft[v].prefactor_ < 0) {
      ss << "(" << ft[v].prefactor_ << ")";
    }

    for (int i = 0; i < level.size(); i++) {

      if (ft[level[i]].operation_ == add) {
        need_brackets = true;
        break;
      }
      if (ft[level[0]].prefactor_ < 0) {
        need_brackets = true;
        break;
      }
      // if(v==get_root(ft) &&
      // ft[get_root(ft)].operation_==mult){need_brackets=true; break;}
    }
    // need_brackets=true;
    if (need_brackets) {
      ss << "(" << pretty_print_level(ft, level, ft[v].operation_) << ")";
...
std::string TamiBase::FermiTree::pretty_print_level(fermi_tree_t &ft,
                                                    std::vector<vertex_t> &vv,
                                                    int source_op) {
  ...
  for (int i = 0; i < vv.size(); i++) {
    ...
    case 2:
      ...
      if (ft[vv[i]].prefactor_ == -1) {
        ss << "-";
      }
      if (ft[vv[i]].prefactor_ == 1 && i != 0) {
        ss << "+";
      }
...
std::string TamiBase::FermiTree::pretty_print_ft(fermi_tree_t &ft1,
                                                 vertex_t &v) {
...
std::string
TamiBase::FermiTree::pretty_print_pole(TamiBase::pole_struct &pole) {
...
void TamiBase::FermiTree::print_vertex(vertex_t &v, fermi_tree_t &ft) {
  ...
  if (ft[v].operation_ == 0) {
    std::cout << " + ";
  }
  if (ft[v].operation_ == 1) {
    std::cout << " x ";
  }
  if (ft[v].operation_ == 2) {
    std::cout << " end ";
  }
